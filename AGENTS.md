...
For `ha-ops-addon`, every block/bunch of changes must bump `ha-ops/config.yaml` version and add a matching `ha-ops/CHANGELOG.md` entry before last of commits. If the work involves series of commits, bump should be performed at the end. The commit that bumps `ha-ops/config.yaml` must also create a Git tag with exactly the same version, for example version `0.6.9` must have tag `0.6.9`. This is required because Home Assistant App updates depend on the version field.
When the user explicitly requests a release, prepare the version bump, changelog, matching plain-version tag, and release commit locally. Publish only by invoking the repository's `./publish-release` script; do not call `git push` directly. The script must verify the expected `origin`, current branch, and matching local tag before it publishes the branch and tag.
...
When a bug is found or fixed, always add or update regression tests that fail on the bug and pass with the fix. Do this automatically; the user should not need to ask for tests. For visual UI behavior, run the affected flow with Playwright, inspect the resulting screenshot or DOM evidence, and keep fixing until the rendered UI matches the user-visible contract; do not treat green non-visual unit tests as sufficient for a visual feature.
...
For UI work, use Vaadin OSS components and Vaadin-first patterns before custom controls. Do not introduce Vaadin Commercial Features unless the user explicitly approves the dependency and licensing impact.
