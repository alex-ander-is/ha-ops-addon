...
`,Dr=[be,Nr,Fn];var ei=class extends et(E(x(_(y(m))))){static get is(){return"vaadin-confirm-dialog-overlay"}static get styles(){return Dr}static get properties(){return{cancelButtonVisible:{type:Boolean,value:!1},rejectButtonVisible:{type:Boolean,value:!1}}}render(){return u`
    ...
    `}_onOpenedChanged(t){this.opened=t.detail.value}};g(ti);var jr=(r="vaadin-details-summary")=>h`
  :host {
    align-items: center;
...
`;var ni=class extends x(E(_(y(m)))){static get is(){return"vaadin-input-container"}static get styles(){return Hr}static get properties(){return{disabled:{type:Boolean,reflectToAttribute:!0},readonly:{type:Boolean,reflectToAttribute:!0},invalid:{type:Boolean,reflectToAttribute:!0}}}render(){return u`
      <slot name="prefix"></slot>
      <slot></slot>
      <slot name="suffix"></slot>
...
`;var ai=class extends Yr(x(E(_(y(m))))){static get is(){return"vaadin-select-list-box"}static get styles(){return Zr}static get properties(){return{orientation:{readOnly:!0}}}get _scrollerElement(){return this.shadowRoot.querySelector('[part="items"]')}render(){return u`
      <div part="items">
        <slot></slot>
      </div>
...
`;var hi=class extends ne(x(_(y(m)))){static get is(){return"vaadin-select-value-button"}static get styles(){return is}render(){return u`
      <div class="vaadin-button-container">
        <span part="label">
          <slot></slot>
        </span>
...
`)}</pre>`}firstUpdated(){let t=this.renderRoot.querySelector("pre"),e=null;try{e=JSON.parse(sessionStorage.getItem("haOpsLogScrollState")||"null")}catch{}requestAnimationFrame(()=>{t.scrollTop=e?.sticky===!1?Math.min(e.scrollTop||0,t.scrollHeight-t.clientHeight):t.scrollHeight}),t.addEventListener("scroll",()=>{let i=t.scrollHeight-t.scrollTop-t.clientHeight<=4;sessionStorage.setItem("haOpsLogScrollState",JSON.stringify({sticky:i,scrollTop:t.scrollTop}))},{passive:!0})}updated(){let t=this.renderRoot.querySelector("pre"),e=null;try{e=JSON.parse(sessionStorage.getItem("haOpsLogScrollState")||"null")}catch{}(!e||e.sticky!==!1)&&requestAnimationFrame(()=>{t.scrollTop=t.scrollHeight})}};customElements.define("ha-ops-log",fi);var mi=class extends m{static properties={path:{type:String},cursor:{type:Object},generation:{type:Number},expanded:{type:Boolean},diff:{type:String},diffState:{type:String},selected:{type:Boolean},choice:{type:String},conflict:{type:Boolean},direction:{type:String},running:{type:Boolean},wrapLines:{type:Boolean}};static styles=h`
  ...
  `;constructor(){super(),this.path="",this.cursor=null,this.generation=0,this.expanded=!1,this.diff="",this.diffState="idle",this.selected=!1,this.choice="",this.conflict=!1,this.direction="apply",this.running=!1,this.wrapLines=!1}willUpdate(t){let e=t.has("cursor")&&ls(t.get("cursor"))!==ls(this.cursor),i=t.has("path")&&t.get("path")!==this.path;(e||t.has("generation")||i)&&(this.expanded=!1,this.diff="",this.diffState="idle")}render(){return u`
      <vaadin-details
        .opened=${this.expanded}
        ?disabled=${this.running}
        @opened-changed=${this.onOpenedChanged}>
        <vaadin-details-summary slot="summary" aria-label=${`${this.path} ${this.expanded?b.collapse:b.expand}`}>
          <div class="summary-row">
            <div class="path">
              <vaadin-checkbox
                label=${b.includeFile||"Include file"}
                aria-label=${`${b.includeFile||"Include file"} ${this.path}`}
            ...
            <div
              class="choice"
              role="group"
              aria-label=${`${b.versionChoice||"Version choice"} ${this.path}`}
              @click=${this.stopTogglePropagation}
              @keydown=${this.stopKeyboardTogglePropagation}>
              <vaadin-button
                ...
                ${this.wrapLines?b.unwrapLines||"Unwrap Lines":b.wrapLines||"Wrap Lines"}
              </vaadin-button>
              ${this.choiceButton("ha",b.useHaVersion)}
              ${this.choiceButton("git",b.useGitVersion)}
            </div>
          </div>
        </vaadin-details-summary>
        ${this.expanded?this.diffState==="loaded"?u`<pre class=${this.wrapLines?"wrap-lines":""} aria-label="Diff detail">${Yn(this.diff)}</pre>`:u`<div role="status">${this.diffState==="stale"?b.unavailableDiff:b.loadingDiff}</div>`:v}
      </vaadin-details>
  ...
  `;constructor(){super(),this.state={},this.direction="apply",this.running=!1,this.wrapByPath={},this.previewIdentityKey="",this.commitSubject="",this.defaultCommitSubject="",this.commitSubjectPreviewIdentityKey=""}get paths(){return this.direction==="save"?this.state.last_save_preview_paths||[]:this.state.last_preview_paths||[]}get cursor(){return this.direction==="save"?this.state.last_save_diff_cursor:this.state.last_diff_cursor}get selectedPaths(){return this.direction==="save"?this.state.save_preview_selected_paths||[]:this.state.apply_preview_selected_paths||[]}get resolutions(){return this.direction==="save"?this.state.save_preview_resolutions||{}:this.state.apply_preview_resolutions||{}}get conflictPaths(){return this.direction==="save"?this.state.last_save_preview_conflict_paths||[]:this.state.last_preview_conflict_paths||[]}get finalCommand(){return this.direction==="save"?"save":"apply"}get finalLabel(){return this.direction==="save"?b.save:b.apply}get selectCommand(){return this.direction==="save"?"select_save_preview":"select_apply_preview"}get resolveCommand(){return this.direction==="save"?"resolve_save_preview":"resolve_apply_preview"}willUpdate(){let t=JSON.stringify(we(this.state,this.direction));t!==this.previewIdentityKey&&(this.previewIdentityKey=t,this.wrapByPath={}),this.direction==="save"&&t!==this.commitSubjectPreviewIdentityKey&&(this.commitSubjectPreviewIdentityKey=t,this.defaultCommitSubject=this.state.last_save_commit_subject||"",this.commitSubject=this.defaultCommitSubject)}isSelected(t){return new Set(this.selectedPaths).has(t)}isConflict(t){return new Set(this.conflictPaths).has(t)}isWrapped(t){return!!this.wrapByPath[t]}allCurrentPathsWrapped(){return this.paths.length>0&&this.paths.every(t=>this.isWrapped(t))}choiceFor(t){return this.resolutions[t]||""}effectiveChoice(t){let e=this.choiceFor(t);return e||(this.direction==="save"&&this.isConflict(t)&&this.isSelected(t)?"":this.direction==="save"?"ha":"git")}selectedConflictChoicesMissing(){if(this.direction!=="save")return!1;let t=new Set(this.selectedPaths);return this.conflictPaths.some(e=>t.has(e)&&!this.resolutions[e])}isFinalActionDisabled(){return this.running||!this.selectedPaths.length||this.selectedConflictChoicesMissing()}render(){return this.paths.length?u`
      <header>
        <h3>${this.direction==="save"?b.savePreview:b.applyPreview}</h3>
        <div class="actions">
          <vaadin-button theme="secondary" @click=${()=>this.wrapAll(!this.allCurrentPathsWrapped())}>
            ${this.allCurrentPathsWrapped()?b.unwrapAllLines||"Unwrap All Lines":b.wrapAllLines||"Wrap All Lines"}
          </vaadin-button>
          <vaadin-button theme="secondary" ?disabled=${this.running} @click=${()=>this.selectAll(!0)}>${b.selectAll}</vaadin-button>
          <vaadin-button theme="secondary" ?disabled=${this.running} @click=${()=>this.selectAll(!1)}>${b.selectNone}</vaadin-button>
          <vaadin-button theme="secondary" ?disabled=${this.running} @click=${()=>this.setAll(!0)}>${b.expandAll}</vaadin-button>
          <vaadin-button theme="secondary" ?disabled=${this.running} @click=${()=>this.setAll(!1)}>${b.collapseAll}</vaadin-button>
        </div>
      </header>
      <div class="files">
    ...
    `:v}wrapAll(t){let e={};for(let i of this.paths)e[i]=t;this.wrapByPath=e}setAll(t){if(!this.running)for(let e of this.renderRoot.querySelectorAll("ha-ops-preview-file"))e.setExpanded(t)}selectAll(t){this.running||this.dispatchEvent(new CustomEvent("ha-ops-command",{bubbles:!0,composed:!0,detail:{command:this.selectCommand,payload:{selection_action:t?"all":"none",preview_identity:we(this.state,this.direction)}}}))}onPreviewSelect=t=>{t.stopPropagation(),!this.running&&this.dispatchEvent(new CustomEvent("ha-ops-command",{bubbles:!0,composed:!0,detail:{command:this.selectCommand,payload:{path:t.detail.path,selected:t.detail.selected?"1":"",preview_identity:we(this.state,this.direction)}}}))};onPreviewResolve=t=>{t.stopPropagation(),!this.running&&this.dispatchEvent(new CustomEvent("ha-ops-command",{bubbles:!0,composed:!0,detail:{command:this.resolveCommand,payload:{path:t.detail.path,choice:t.detail.choice,preview_identity:we(this.state,this.direction)}}}))};onPreviewWrapToggle=t=>{t.stopPropagation(),this.wrapByPath={...this.wrapByPath,[t.detail.path]:!!t.detail.wrapLines}};onCommitSubjectInput=t=>{this.commitSubject=t.target.value};runFinalAction(){if(this.isFinalActionDisabled())return;let t=this.direction==="save"?{commit_subject:this.commitSubject,default_commit_subject:this.defaultCommitSubject}:{};this.dispatchEvent(new CustomEvent("ha-ops-command",{bubbles:!0,composed:!0,detail:{command:this.finalCommand,payload:t}}))}};customElements.define("ha-ops-preview",vi);var gi=class extends m{static properties={connection:{type:String},revision:{type:Number},state:{type:Object},confirmOpen:{type:Boolean},confirmMessage:{type:String}};static styles=h`
    :host { display: contents; }
