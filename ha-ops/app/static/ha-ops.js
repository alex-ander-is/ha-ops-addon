...
`,Fs=[be,js,Xr];var ii=class extends tt(E(C(b(w(v))))){static get is(){return"vaadin-confirm-dialog-overlay"}static get styles(){return Fs}static get properties(){return{cancelButtonVisible:{type:Boolean,value:!1},rejectButtonVisible:{type:Boolean,value:!1}}}render(){return h`
...
`;var pi=class extends re(C(b(w(v)))){static get is(){return"vaadin-select-value-button"}static get styles(){return an}render(){return h`
      <div class="vaadin-button-container">
        <span part="label">
          <slot></slot>
        </span>
...
`)}],identifiers:["identifiers",c.identifiers,o=>(o.recovered_identifiers||[]).slice(0,3).map(a=>Array.isArray(a)?a.join(":"):String(a)).join(", ")],"original-name":["original-name",c.originalName,o=>o.original_name||""],source:["source",c.source,o=>[String(o.source_commit||"").slice(0,12),o.source_path].filter(Boolean).join(" ")]},e=["id","original-name","area","device"],i=["identifiers","name","entity-id","source"],n=(o,a)=>o.map(l=>{let[d,p]=t[l];return h`<div class=${`deleted-device-header-cell deleted-device-cell-${a} deleted-device-col-${l}`}>${p}</div>`}),r=(o,a,l)=>o.map(d=>{let[p,f,x]=t[d],y=String(x(a));return h`<div class=${`deleted-device-cell deleted-device-cell-${l} deleted-device-cell-${d} deleted-device-col-${d}`}>
  ...
  `}function fo(s){return s?.entity_id||s?.name||s?.id||""}function mi(s,t){return t?.length?h`<p class="deleted-entity-label">${s}</p><ul>${t.map(e=>h`<li>${fo(e)}</li>`)}</ul>`:g}function _n(s){if(!s||typeof s!="object")return h`<p>${c.noDeletedDevices}</p>`;let t=s.device_groups||[],e=s.orphan_entity_groups||[];return!t.length&&!e.length?h`<p>${c.noDeletedDevices}</p>`:h`
    <div class="deleted-devices-tree">
      ...
      ${t.map(i=>{let n=i.device||{},r=i.counts||{},o=[n.manufacturer,n.model,n.model_id].filter(Boolean).join(" / "),a=[n.label||n.id||c.deletedDevicesLabel,o,n.area].filter(Boolean).join(" \xB7 "),l=Number(r.deleted_entities||0),d=Number(r.active_entities||0),p=[(c.deletedDeviceGroupRemoveCount||"{deleted} entities to remove").replace("{deleted}",String(l))];d>0&&p.push((c.deletedDeviceGroupActiveCount||"{active} active entities").replace("{active}",String(d)));let f=p.join(", "),x=[String(n.source_commit||"").slice(0,12),n.source_path].filter(Boolean).join(" "),y=(n.identifiers||[]).slice(0,3).map($=>Array.isArray($)?$.join(":"):String($)).join(", ");return h`
